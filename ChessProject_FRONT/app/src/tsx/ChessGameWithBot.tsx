...
import styles from '../css/ChessGameWithBot.module.css'; // Importez votre feuille de style
...
const ChessGameWithBot: React.FC<{ userName: string }> = ({ userName }) => {
  const [game] = useState(new Chess());
  const [isBotTurn, setIsBotTurn] = useState(false);
  const [fen, setFen] = useState(game.fen());
  const [playerTime, setPlayerTime] = useState(2 * 180);
  const [botTime, setBotTime] = useState(2 * 180);
...
export default ChessGameWithBot;
